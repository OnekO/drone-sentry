...
git clone https://${PLUGIN_REPO_USER}:${PLUGIN_REPO_TOKEN}@github.com/${PLUGIN_REPO_ORG}/${DRONE_REPO_NAME}.git && cd ${DRONE_REPO_NAME}
git remote set-url origin https://${PLUGIN_REPO_USER}:${PLUGIN_REPO_TOKEN}@github.com/${PLUGIN_REPO_ORG}/${DRONE_REPO_NAME}.git
git checkout ${DRONE_SOURCE_BRANCH}
echo "Creando release ${SENTRY_RELEASE}"
sentry-cli releases new -p $SENTRY_PROJECT $SENTRY_RELEASE
echo "Commits para ${SENTRY_RELEASE}"
sentry-cli releases set-commits $SENTRY_RELEASE --auto
if [ "$PLUGIN_SOURCEMAPS" == "true" ]
then
